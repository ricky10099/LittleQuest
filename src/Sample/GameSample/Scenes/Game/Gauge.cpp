...
namespace Sample::GameSample {
    namespace HP {
        ...
        void Gauge::Draw() {
            ...
            // ダメージ用
            DrawBox(x, y, x + static_cast<int>(BAR_W * ToPerValue(damage_)),
                    y + BAR_H, BAR_DAMAGE_COLOR, TRUE);

            if (recover_delta_) {
                // 回復用
                DrawBox(x, y,
                        x + static_cast<int>(BAR_W * ToPerValue(recover_)),
                        y + BAR_H, BAR_NORMAL_COLOR, TRUE);
                // 現在のHP
                DrawBox(x, y, x + static_cast<int>(BAR_W * ToPerValue(hp_)),
                        y + BAR_H, BAR_RECOVER_COLOR, TRUE);
            } else {
                // 現在のHP
                DrawBox(x, y, x + static_cast<int>(BAR_W * ToPerValue(hp_)),
                        y + BAR_H, BAR_NORMAL_COLOR, TRUE);

...
namespace {
    ...
    class CharacterContactListener
        : public physics::Character::ContactListener {
            ...
            virtual void onContactAdded(
                ...
                auto it = std::find(prop_body_id_list_.begin(),
                                    prop_body_id_list_.end(), other_body_id);
                if (it != prop_body_id_list_.end()) {
                    size_t index =
                        it
                        - prop_body_id_list_.begin();    // 配列インデックス番号
...
bool ScenePhysicsLayer::Init() {
    ...
    // オブジェクトレイヤーをカスタム設定
    //==========================================================
    physics::Engine::instance()->setLayerAlias(myLayerTable_,
                                               std::size(myLayerTable_));
    ...
    camera->SetPositionAndTarget(float3(0.0f, 2.0f, -10.0f),
                                 {0.0f, 1.0f, 0.0f});
...
void ScenePhysicsLayer::Update() {
    ...
    //----------------------------------------------------------
    {
        ...
        float3 new_velocity   = character_->move(delta, move_direction,
                                                 old_position, jump_direction);
...
void ScenePhysicsLayer::Draw() {
    ...
    DrawFormatString(100, 50, GetColor(255, 255, 255),
                     "[Physic] 衝突レイヤー分けサンプル");
    ...
    DrawCapsule3D(
        cast(position + float3(0.0f, 1.f, 0.0f) * s->radius_),
        cast(position
             + float3(0.0f, 1.f, 0.0f) * (s->radius_ + s->half_height_ * 2.0f)),
        s->radius_, 8, GetColor(0, 255, 0), GetColor(0, 0, 0), true);

    //----------------------------------------------------------
    // 設置物の状態を表示
    //----------------------------------------------------------
    auto put_state = [](const float3& position, const char* message,
                        u32 color) {
    ...
    put_state(rigid_body0_->position() + float3(0.0f, 1.0f, 0.0f),
              "押されない\n押せない", GetColor(255, 255, 255));
    put_state(rigid_body1_->position() + float3(0.0f, 1.0f, 0.0f),
              "押されない\n押せる", GetColor(255, 255, 255));
    put_state(rigid_body2_->position() + float3(0.0f, 1.0f, 0.0f),
              "押される\n押せる", GetColor(255, 255, 255));
    put_state(rigid_body3_->position() + float3(0.0f, 1.0f, 0.0f),
              "押される\n押せない", GetColor(255, 255, 255));

    put_state(rigid_body0_->position() + float3(0.0f, 2.5f, 0.0f), "当たらない",
              GetColor(255, 64, 64));
    put_state(rigid_body1_->position() + float3(0.0f, 2.5f, 0.0f), "当たる",
              GetColor(64, 255, 64));
    put_state(rigid_body2_->position() + float3(0.0f, 2.5f, 0.0f), "当たる",
              GetColor(64, 255, 64));
    put_state(rigid_body3_->position() + float3(0.0f, 2.5f, 0.0f), "当たる",
              GetColor(64, 255, 64));
    ...
    //----------------------------------------------------------
    {
        // 書式付き文字列の描画幅・高さ・行数を取得する
        const char* message =
            "[W/A/S/D] 移動 [SPACE] 物理シミュレーションをリセット";
        s32 width;         // 幅
        s32 height;        // 高さ
        s32 line_count;    // 行数
        GetDrawFormatStringSize(&width, &height, &line_count, message);
...
void ScenePhysicsLayer::ResetPhysics() {
    ...
    // ボックス
    {
        ...
        // PBRテクスチャで上書き
        auto tex_albedo_ =
            std::make_shared<Texture>(path + "DefaultMaterial_Base_color.png");
        auto tex_normal_ = std::make_shared<Texture>(
            path + "DefaultMaterial_Normal_DirectX.png");
        auto tex_roughness_ =
            std::make_shared<Texture>(path + "DefaultMaterial_Roughness.png");
        auto tex_metalness_ =
            std::make_shared<Texture>(path + "DefaultMaterial_Metallic.png");
        auto tex_ao_ =
            std::make_shared<Texture>(path + "DefaultMaterial_Mixed_AO.png");

        auto model =
            std::make_shared<Model>("data/Sample/Sci-fi_Box/Sci-fi Box.mv1");
    ...
    // コンテナボックス
    {
        ...
        auto tex_albedo_ = std::make_shared<Texture>(
            path + "Sci-fi_Box_AlbedoTransparency.png");
        auto tex_normal_ =
            std::make_shared<Texture>(path + "Sci-fi_Box_Normal_DirectX.png");
        auto tex_roughness_ = std::make_shared<Texture>(
            path + "Sci-fi_Box_MetallicSmoothness.png");
        auto tex_metalness_ = std::make_shared<Texture>(
            path + "Sci-fi_Box_MetallicSmoothness.png");
        auto tex_ao_ = std::make_shared<Texture>(path + "Sci-fi_Box_AO.png");

        auto model = std::make_shared<Model>(
            "data/Sample/Sci-fi_Container/Sci-fi Container.mv1");
    ...
    // ドラム缶
    {
        ...
        auto tex_albedo_ =
            std::make_shared<Texture>(path + "drum2_base_color.png");
        auto tex_normal_ = std::make_shared<Texture>(path + "drum2_normal.png");
        auto tex_roughness_ =
            std::make_shared<Texture>(path + "drum2_roughness.png");
        auto tex_metalness_ =
            std::make_shared<Texture>(path + "drum2_metallic.png");
        auto tex_ao_ = std::make_shared<Texture>(path + "drum2_ambient.png");

        auto model =
            std::make_shared<Model>("data/Sample/oil_barrels_pbr/barrel.mv1");
    ...
    // 三角コーン
    {
        ...
        auto tex_albedo_ = std::make_shared<Texture>(
            path + "Red_Dirty_Traffic Cone_Base_Color.png");
        auto tex_normal_ = std::make_shared<Texture>(
            path + "Dirty_Traffic Cone_Normal_DirectX.png");
        auto tex_roughness_ = std::make_shared<Texture>(
            path + "Dirty_Traffic Cone_Roughness.png");
        auto tex_metalness_ =
            std::make_shared<Texture>(path + "Dirty_Traffic Cone_Metallic.png");
        auto tex_ao_ =
            std::make_shared<Texture>(path + "Dirty_Traffic Cone_AO.png");

        auto model = std::make_shared<Model>(
            "data/Sample/Traffic_Cone/Traffic Cone.mv1");
    ...
    // 剛体を作成
    //==========================================================
    rigid_body0_ = physics::createRigidBody(
        shape::Box{float3(0.5f, 0.5f, 0.5f)}, MyLayer::Prop0);
    rigid_body1_ = physics::createRigidBody(
        shape::Box{float3(0.5f, 0.5f, 0.5f)}, MyLayer::Prop1);
    rigid_body2_ =
        physics::createRigidBody(shape::Cylinder{0.5f, 0.375f}, MyLayer::Prop2);
    rigid_body3_ = physics::createRigidBody(
        shape::ConvexHull(model_cone_.get()), MyLayer::Prop3);
    ...
    //==========================================================
    character_ =
        physics::createCharacter(MyLayer::Player);    // [Layer] プレイヤー
    ...
    character_->setShape(
        shape_standing_,
        float3(0.0f, shape_standing_.half_height_ + shape_standing_.radius_,
               0.0f));

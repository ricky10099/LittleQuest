...
namespace Sample {
    ...
    bool SceneTest1::Init() {
        ...
        auto obj2 = Scene::CreateObjectPtr<Object>()->SetName(
            "obj2");    //< オブジェクト作成
        obj2->AddComponent<
            ComponentCollisionSphere>();    //< 作成時につなげてしまうと
        obj2->AddComponent<
            ComponentCamera>();    //<
...

        {
            ...
            // モデルコンポーネント
            auto comp_boss = obj_boss->AddComponent<ComponentModel>(
                "data/Sample/Boss/model.mv1");
            // モデルを 0.05倍にする (普通の書き方)
            comp_boss->SetScaleAxisXYZ({0.05f});

            // コンポーネントもそのまま処理できます。
            // (0,2,0)の位置にCollisionSphereを移動させています
            obj_boss->AddComponent<ComponentCollisionSphere>()->SetTranslate(
                {0, -2, 0});
    ...
    void SceneTest1::Draw() {
        ...
        DrawFormatString(100, 50, GetColor(255, 255, 255), "Windos11 : %d, %d",

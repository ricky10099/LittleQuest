...
bool ScenePhysics::Init() {
    ...
    camera->SetPositionAndTarget(float3(0.0f, 6.0f, -15.0f),
                                 {0.0f, 5.0f, 0.0f});
...
void ScenePhysics::ResetPhysics() {
        ...
        for (s32 y = 0; y < HEIGHT; ++y) {
            for (s32 x = 0; x < WIDTH; ++x) {
                auto model = std::make_shared<Model>(
                    "data/Sample/Sci-fi_Box/Sci-fi Box.mv1");

                // モデルに設定されているテクスチャを上書き
                model->overrideTexture(Model::TextureType::Diffuse,
                                       tex_albedo_);
                model->overrideTexture(Model::TextureType::Normal, tex_normal_);
                model->overrideTexture(Model::TextureType::Roughness,
                                       tex_roughness_);
                model->overrideTexture(Model::TextureType::Metalness,
                                       tex_metalness_);
    ...
    // ドラム缶
    {
        ...
        for (s32 y = 0; y < HEIGHT; ++y) {
            for (s32 x = 0; x < WIDTH; ++x) {
                auto model = std::make_shared<Model>(
                    "data/Sample/oil_barrels_pbr/barrel.mv1");
    ...
    for (s32 y = 0; y < HEIGHT; ++y) {
        for (s32 x = 0; x < WIDTH; ++x) {
            ...
            switch (type) {
                case 0:
                    rigid_body = physics::createRigidBody(
                        shape::Box{float3(0.5f, 0.5f, 0.5f)},
                ...
                case 2:
                    rigid_body =
                        physics::createRigidBody(shape::Cylinder{0.5f, 0.375f},
                                                 physics::ObjectLayers::MOVING);
                    break;
                case 3:
                    rigid_body = physics::createRigidBody(
                        shape::ConvexHull(model_cone_[0].get()),
                        physics::ObjectLayers::MOVING);
                    break;
                default:
                    break;
            ...
            float3 position =
                float3((x - WIDTH / 2) * 1.5f, y * 1.5f + 2.0f, 0.0f);

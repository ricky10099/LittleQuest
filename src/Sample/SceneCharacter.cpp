...
namespace {
    ...
    shape::Capsule shape_standing_ =
        shape::Capsule(0.8f, 0.3f);    //!< 立っている時
    shape::Capsule shape_crouching_ =
        shape::Capsule(0.4f, 0.3f);    //!< しゃがんでいる時
...
bool SceneCharacter::Init() {
    ...
    //-------------------------------------------------------
    model_env_ =
        std::make_shared<Model>("data/Sample/空色町1.52/sorairo1.52.mv1");

    // 地形衝突情報を作成 (メッシュ剛体は必ず静的)
    body_env_ = physics::createRigidBody(
        shape::Mesh(model_env_.get(), 1.0f),    // メッシュ
    ...
    camera->SetPositionAndTarget(float3(-15.0f, 12.0f, -15.0f),
                                 {0.0f, 5.0f, 0.0f});
...
void SceneCharacter::Update() {
    ...
    // 常に切り替えると階段昇降がぎこちなくなる
    {
        ...
        if (!old_key && key) {
            ...
            character_->setShape(
                shape_crouching_,
                float3(0.0f,
                       shape_crouching_.half_height_ + shape_crouching_.radius_,
                       0.0f));
        }
        ...
        if (old_key && !key) {
            ...
            character_->setShape(
                shape_standing_,
                float3(0.0f,
                       shape_standing_.half_height_ + shape_standing_.radius_,
                       0.0f));
        }
    ...
    // 移動と背景衝突補正
    float3 new_velocity =
        character_->move(delta, move_direction, old_position, jump_direction);

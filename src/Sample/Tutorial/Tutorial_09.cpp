...
namespace Tutorial {
    ...
    class ComponentSamplePlayerController : public Component {
        public:
            ...
            void Update() override {
                ...
                if (IsKey(KEY_INPUT_UP)) owner->AddTranslate(mat.axisZ());

                if (IsKey(KEY_INPUT_DOWN)) owner->AddTranslate(-mat.axisZ());

                if (IsKey(KEY_INPUT_RIGHT)) owner->AddTranslate(mat.axisX());

                if (IsKey(KEY_INPUT_LEFT)) owner->AddTranslate(-mat.axisX());
            }
    ...
    BP_COMPONENT_IMPL(ComponentSamplePlayerController,
                      u8"プレイヤー移動サンプルコンポーネント");
    ...
    bool Tutorial_09::Init() {
        ...
        obj->AddComponent<ComponentModel>(
               "data/Sample/Player/model.mv1")    // モデル
    ...
    void Tutorial_09::GUI() {
        ...
        ImGui::TextColored(
            {0, 1, 0, 1},
            u8"AddComponentするとカメラに対してカーソル移動できます");
        ImGui::Spacing();
        ImGui::Separator();
    }
...
CEREAL_REGISTER_POLYMORPHIC_RELATION(Tutorial::ComponentSamplePlayerController,
                                     Component)

...
namespace Tutorial {
    ...
    namespace {
        void FadeIn() {
            Scene::CreateObjectPtr<Object>()    // フェード用オブジェクト
                ->SetName("FadeIn")             // 名前設定
                ->AddComponent<
                    ComponentFilterFade>()    // フェードコンポーネント
        ...
        bool WaitFadeIn() {
            if (auto obj = Scene::GetObjectPtr<Object>("FadeIn")) {
                ...
                if (fade->IsFadeIn()) {
                    if (!fade->IsFinishFade()) return true;
    ...
    bool Tutorial_12::Init() {
        ...
        Scene::CreateObjectPtr<
            ...
            ->SetPositionAndTarget(
                {0, 5, 0},
                {0, 30, 100});    //< SpringArmとコリジョンを使う場合はこれで
    ...
    void Tutorial_12::Draw() {
        DrawFormatString(100, 100, GetColor(255, 255, 255), "Scene : %s",
                         Scene::GetCurrentScene()->GetName().data());
    }

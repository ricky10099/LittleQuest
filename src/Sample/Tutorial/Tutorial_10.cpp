    ...
    class Tutorial_10 : public Scene::Base {
        public:
            ...
            bool Init() override {
                ...
                // ▣データをセーブする
                // S1.
                // 以下のシーケンサなどで作成されたオブジェクトをGUIで選択して「Save」を押します
                // S2. data/_save/object/(オブジェクト名).txt　にセーブされます
                //
                // ▣データをロードする
                // L1. プログラム上で、オブジェクト->Load("オブジェクト名");
                // とする
                ...
                //-------------------------------------------------------------------------------
                auto sequencer =
                    Scene::CreateObjectPtr<Object>()    //< シーケンサを作る
                        ->SetName("Sequencer");
                ...
                if (sequencer->Load("Sequencer"))    //< L1
                {
                    ...
                    if (sequencer =
                            Scene::GetObjectPtr<Object>("Sequencer"))    //< P1
                    {
                        //再生する
                        if (auto seq = sequencer->GetComponent<
                                       ComponentSequencer>())    //< P2
                            seq->Play();
                    }
                } else {

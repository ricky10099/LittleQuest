        ...
        public:
            ...
            bool Init() override {
                ...
                obj->AddComponent<ComponentModel>(
                    ...
                    ->SetAnimation({
                        {"idle", "data/Sample/Player/Anim/Idle2.mv1", 0,
                         1.0f},    // Idle
                        {"jump", "data/Sample/Player/Anim/Jump2.mv1", 0,
                         1.0f}    // Jump
                    });
                ...
                obj->AddComponent<ComponentObjectController>()->SetMoveSpeed(
                    2.0f);
                ...
                boss->AddComponent<ComponentModel>("data/Sample/Boss/model.mv1")
                    ->SetAnimation({
                        {"idle", "data/Sample/Player/Anim/Idle.mv1", 0,
                         1.0f},    // Idle
                        {"idle", "data/Sample/Player/Anim/Walk.mv1", 0,
                         1.0f},    // Walk
                        {"jump", "data/Sample/Player/Anim/Death.mv1", 0,
                         1.0f}    // Deah
                    });
                ...
                cam_obj->AddComponent<ComponentCollisionSphere>()
                    ...
                    ->SetRadius(1.0f)
                    ->SetCollisionGroup(
                        ComponentCollision::CollisionGroup::CAMERA)
            ...
            void Update() override {
                ...
                // ボスか操作キャラがいなければ何もしない
                if (boss == nullptr || mouse == nullptr) return;

                if (auto ctrl =
                        mouse->GetComponent<ComponentObjectController>()) {
                    // 距離が100以内
                    float len =
                        length(boss->GetTranslate() - mouse->GetTranslate());
                    if (len < 100) {
                        // Bossの方に向ける
                        ctrl->SetTarget(boss);
                    } else {

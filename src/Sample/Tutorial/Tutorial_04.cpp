    ...
    void Tutorial_04::Draw() {
        ...
        for (int i = 0; i < button.size(); ++i) {
            ...
            DrawFormatString(ofs.x, ofs.y + ((line++) * 32),
                             GetColor(0, 255, 255), "%s\n", button[i]);
            DrawFormatString(ofs.x, ofs.y + ((line++) * 32), col,
                             "IsMouseDown   : %4d", mouse_[i].count_down);
            DrawFormatString(ofs.x, ofs.y + ((line++) * 32), col,
                             "IsMouseUp     : %4d", mouse_[i].count_up);
            DrawFormatString(ofs.x, ofs.y + ((line++) * 32), col,
                             "IsMouse       : %4d", mouse_[i].count);

            DrawFormatString(ofs.x, ofs.y + ((line++) * 32), col,
                             "IsMouseOn     : %4d", mouse_[i].count_down);
            DrawFormatString(ofs.x, ofs.y + ((line++) * 32), col,
                             "IsMouseRelease: %4d", mouse_[i].count_release);
            DrawFormatString(ofs.x, ofs.y + ((line++) * 32), col,
                             "IsMouseRepeat : %4d", mouse_[i].count_repeat);
        }
    }

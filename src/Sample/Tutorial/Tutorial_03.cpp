...
namespace Tutorial {
    ...
    void Tutorial_03::Draw() {
        ...
        int1 line      = 3;
        DrawFormatString(ofs.x, ofs.y + ((line++) * 32), GetColor(0, 255, 0),
                         "KEY_INPUT_SPACE", count_down_);
        DrawFormatString(ofs.x, ofs.y + ((line++) * 32),
                         GetColor(255, 255, 255), "IsKey        : %4d", count_);
        DrawFormatString(ofs.x, ofs.y + ((line++) * 32),
                         GetColor(255, 255, 255), "IsKeyDown    : %4d",
                         count_down_);
        DrawFormatString(ofs.x, ofs.y + ((line++) * 32),
                         GetColor(255, 255, 255), "IsKeyUp      : %4d",
                         count_up_);

        DrawFormatString(ofs.x, ofs.y + ((line++) * 32),
                         GetColor(255, 255, 255), "IsKeyOn      : %4d",
                         count_on_);
        DrawFormatString(ofs.x, ofs.y + ((line++) * 32),
                         GetColor(255, 255, 255), "IsKeyRelease : %4d",
                         count_release_);
        DrawFormatString(ofs.x, ofs.y + ((line++) * 32),
                         GetColor(255, 255, 255), "IsKeyRepeat  : %4d",
                         count_repeat_);
    }
    ...
    void Tutorial_03::GUI() {
        ...
        ImGui::Begin("HowToUse");
        {
            ...
            ImGui::Text(
                ...
                u8"\n"
                u8"IsKeyOn( 調べたいキー )     :  "
                u8"現在のフレームで押したか判定できる【IsKeyDown()と同じ】\n"
                u8"IsKeyRelase( 調べたいキー ) :  押してないかどうか判定できる "
                u8"     【!IsKey()と同じ】\n"
                u8"IsKeyRepeat( 調べたいキー ) :  "
                u8"2フレーム以上押したか判定できる\n"
                u8"\n"
                u8"調べたいキー:\n"

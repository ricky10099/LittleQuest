...
namespace Sample {
    ...
    namespace HP {
        ...
        class HpGauge {
            public:
                ...
                void Update() {
                    --damage_frame_;
                    if (damage_frame_ > 0) return;
                    // ダメージ分をちょっとだけ見せてから減らす処理する
                    damage_ -= static_cast<int>(
                        static_cast<float>(damage_ - hp_) / 30.f);
                }
                void Draw() {
                    int x = (int)pos_.x;
                    int y = (int)pos_.z;
                    ...
                    // 下地
                    DrawBox(x, y, x + HP_BAR_W, y + HP_BAR_H,
                            GetColor(75, 75, 75), TRUE);
                    // ダメージ用
                    float bar2_per = (damage_ / (float)org_hp_) * 100 * 0.01f;
                    DrawBox(x, y, x + static_cast<int>(HP_BAR_W * bar2_per),
                            y + HP_BAR_H, RED, TRUE);
                    // 現在のHP
                    float bar_per = (hp_ / (float)org_hp_) * 100 * 0.01f;
                    DrawBox(x, y, x + static_cast<int>(HP_BAR_W * bar_per),
                            y + HP_BAR_H, SKYBLUE, TRUE);
                    // フチ
                ...
                void operator+=(int value) {
                    if (hp_ >= org_hp_) return;

                    // 増やせる分がある場合のみ以下を実行
                    hp_ += value;
                    if (hp_ >= org_hp_) hp_ = org_hp_;
    ...
    class Shot : public Object {
        public:
            ...
            void Draw() {
                DrawSphere3D(cast(GetTranslate()), radius_, 10,
                             GetColor(255, 0, 0), GetColor(255, 0, 0), TRUE);
            }

            void OnHit([[maybe_unused]] const ComponentCollision::HitInfo&
                           hitInfo) override {
        ...
        public:
            ...
            bool Init() override {
                ...
                model->SetAnimation(
                    {{"idle", "data/Sample/Player/Anim/Idle.mv1", 1, 1.0f},
                     ...
                     {"dance5", "data/Sample/Player/Anim/Dance5.mv1", 0,
                      1.0f}});
			...
			if( near_enemy )
			{
                ...
                if (IsKeyOn(KEY_INPUT_SPACE)) {
                    ...
                    if (mdl)
                        vec = mul(-float4(mdl->GetMatrix().axisZ(), 0),
                                  GetMatrix())
                                  .xyz;

                    Scene::CreateObjectPtr<Shot>()->SetPositionAndDirection(
                        GetTranslate() + float3(0, 5, 0), vec);
                }
            }
            ...
            void GUI() override {
                ...
                ImGui::Begin(GetName().data());
                {
                    ...
                    ImGui::DragFloat(u8"速度", &speed_, 0.01f, 0.01f, 10.0f,
                                     "%2.2f");
    ...
    bool SceneTest2::Init() {
...
#ifdef USE_STAGE_FPS
        {
            ...
            obj->AddComponent<ComponentModel>(
                   "data/Sample/FPS_Industrial/Map.mv1")
                ->SetScaleAxisXYZ({5, 5, 5});
    ...
    void SceneTest2::Update() {
        if (IsKeyOn(KEY_INPUT_1)) Scene::SetCurrentCamera("PlayerCamera");

        if (IsKeyOn(KEY_INPUT_2)) Scene::SetCurrentCamera("EnemyCamera");

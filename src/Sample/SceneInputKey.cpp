    ...
    class Player : public Object {
        ...
        public:
            bool Init() override {
                ...
                // モデルの追加
                model_ =
                    std::make_shared<Model>("data/Sample/Player/model.mv1");
                // モデルを縮小表示
                matrix mat_world =
                    mul(matrix::scale(0.03f), matrix::translate(position_));
                ...
                // アニメーション初期化
                animation_ =
                    std::make_shared<Animation>(desc_, std::size(desc_));
                ...
                // エフェクト
                //-------------------------------------------------------
                effect_handle_ = LoadEffekseerEffect(
                    "data/Sample/Effects/MagicFire1.efkefc");
                effect_play_handle_ = PlayEffekseer3DEffect(effect_handle_);
                // ひとまずプレイヤー座標にエフェクト出す
                SetPosPlayingEffekseer3DEffect(effect_play_handle_, position_.x,
            ...
            virtual void Update() override {
                ...
                if (IsKeyRelease(KEY_INPUT_W) && IsKeyRelease(KEY_INPUT_S)
                    && is_jump_ == false) {
                ...
                //-------------------------------------------------------
                {
                    ...
                    auto attach_frame =
                        MV1SearchFrame(*model_, "mixamorig:RightHandIndex1");
                    auto right_arm_mat =
                        MV1GetFrameLocalWorldMatrix(*model_, attach_frame);
                    float3 effect_pos =
                        mul(float4(position_, 1.0f), cast(right_arm_mat)).xyz;

                    // エフェクトのループ再生
                    if (IsEffekseer3DEffectPlaying(effect_play_handle_)) {
                        // 再生終了でここに来る
                        effect_play_handle_ =
                            PlayEffekseer3DEffect(effect_handle_);
                    }

                    SetPosPlayingEffekseer3DEffect(effect_play_handle_,
                                                   effect_pos.x, effect_pos.y,
                                                   effect_pos.z);
                }
            }

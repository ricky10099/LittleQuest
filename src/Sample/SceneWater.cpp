...
namespace {
    ...
    class Water {
        public:
            ...
            Water(u32 width, u32 height, const float3& position,
                  f32 scale = 1.0f)
                : width_(width),
                  height_(height),
                  position_(position),
            ...
            void update([[maybe_unused]] f32 dt) {
                ...
                // 二次元配列アクセスのための短縮表記
                auto w = [&](s32 x, s32 y) {
                    return water_height_[y * width_ + x];
                };

                f32 velocity2 =
                    std::clamp(0.125f, 0.0f, 0.5f);    // 速度の2乗 [0.0f-0.5f]
                ...
                for (s32 y = 1; y < height_ - 1; ++y) {
                    for (s32 x = 1; x < width_ - 1; ++x) {
                        ...
                        f32 neighbor = w(x + 1, y).x + w(x - 1, y).x
                                       + w(x, y + 1).x + w(x, y - 1).x;

                        float height =
                            2.0f * center.x - center.g
                            + velocity2 * (neighbor - 4.0f * center.x);
                        center.y = center.x;
                        center.x = height;
            ...
            void render() {
                // 二次元配列アクセスのための短縮表記
                auto w = [&](s32 x, s32 y) {
                    return water_height_[y * width_ + x].x;
                };
                ...
                for (s32 iy = 0; iy < height_; ++iy) {
                    ...
                    // LineStripで描画
                    DrawPrimitive3D(v.data(), width_, DX_PRIMTYPE_LINESTRIP,
                                    DX_NONE_GRAPH, false);
                }
                ...
                for (s32 ix = 0; ix < width_; ++ix) {
                    ...
                    // LineStripで描画
                    DrawPrimitive3D(v.data(), height_, DX_PRIMTYPE_LINESTRIP,
                                    DX_NONE_GRAPH, false);
                }
            ...
            //! 水面の高さを参照
            f32& water(s32 x, s32 y) {
                return water_height_[y * width_ + x].f32[0];
            };
...
void SceneWater::Update() {
    ...
    if (IsMouseRepeat(MOUSE_INPUT_LEFT)) {    // 左クリックしている間
        ...
        float2 uv = float2(static_cast<f32>(x) / WINDOW_W,
                           static_cast<f32>(y) / WINDOW_H);
        float2 screen_position =
            (uv - 0.5f)
            * float2(2.0f, -2.0f);    // UV座標系 → スクリーン座標系への変換
        ...
        // 水面よりも少し下の位置を指定
        //------------------------------------------------------
        bool result = water_->addImpulse(
            water_position + float3(0.0f, water_->position().y - 0.5f, 0.0f),
            0.125f);

        if (result) {
            // クリック先をデバッグ表示(赤色の球を表示)
            SetUseLighting(false);
            DrawSphere3D(cast(water_position), 0.25f, 8, GetColor(255, 0, 0), 0,
                         false);
            SetUseLighting(true);
        }
    }

...
class ObjectMouse : public Object {
    public:
        bool Init() {
            ...
            // コリジョン(カプセル)
            auto col1 =
                AddComponent<ComponentCollisionCapsule>()    //< コリジョン
                    ->SetTranslate({0, 0, 0})
                    ->SetHeight(6.0f);
            ...
            auto col2 =
                AddComponent<ComponentCollisionSphere>()    //< コリジョン
                    ->SetTranslate({0, 0, 0})
                    ->SetRadius(0.5);
        ...
        void OnHit([[maybe_unused]] const ComponentCollision::HitInfo& hitInfo)
            override {
            // 次のownerのオブジェクトと当たった!
            ...
            printfDx("\nHit %s(%s) <=> %s(%s)", owner1->GetName().data(),
                     colname1.data(), owner2->GetName().data(),
                     colname2.data());
...
bool SceneTestCollision::Init() {
    ...
    //----------------------------------------------------------------------------------
    {
        ...
        obj->AddComponent<ComponentModel>("data/Sample/FPS_Industrial/Map.mv1")
            ->SetScaleAxisXYZ({2, 2, 2});
        // obj->AddComponent<ComponentModel>(

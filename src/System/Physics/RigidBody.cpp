...
namespace physics {
    ...
    class RigidBodyImpl : public physics::RigidBody {
        public:
            RigidBodyImpl(const JPH::ShapeRefC& shape, u16 layer,
                ...
                jph_body_ =
                    physics::Engine::bodyInterface()->CreateBody(body_settings);
                ...
                physics::Engine::bodyInterface()->AddBody(
                    jph_body_->GetID(), JPH::EActivation::Activate);
            ...
            virtual void setLayer(u16 layer) override {
                physics::Engine::bodyInterface()->SetObjectLayer(body_id_,
                                                                 layer);
            }

            //! オブジェクトレイヤーを取得
            virtual u16 layer() const override {
                return physics::Engine::bodyInterface()->GetObjectLayer(
                    body_id_);
            }
            ...
            virtual float3 position() const override {
                JPH::Vec3 p =
                    physics::Engine::bodyInterface()->GetPosition(body_id_);
                return castJPH(p);
            }
            ...
            virtual quaternion rotation() const override {
                JPH::QuatArg q =
                    physics::Engine::bodyInterface()->GetRotation(body_id_);
                return castJPH(q);
            }
            ...
            virtual matrix worldMatrix() const override {
                return castJPH(
                    physics::Engine::bodyInterface()->GetWorldTransform(
                        body_id_));
            }
            ...
            virtual void moveKinematic(const float3& target_position,
                ...
                physics::Engine::bodyInterface()->MoveKinematic(
                    body_id_, tpos, trot, delta_time);
            }
            ...
            virtual float3 linearVelocity() const override {
                return castJPH(
                    physics::Engine::bodyInterface()->GetLinearVelocity(
                        body_id_));
            }
            ...
            virtual void setAngularVelocity(const float3& vrot) override {
                physics::Engine::bodyInterface()->SetAngularVelocity(
                    body_id_, castJPH(vrot));
            }
            ...
            virtual float3 pointVelocity(const float3& point) const override {
                return castJPH(
                    physics::Engine::bodyInterface()->GetPointVelocity(
                        body_id_, castJPH(point)));
            }
            ...
            virtual void addForce(const float3& force,
                ...
                physics::Engine::bodyInterface()->AddForce(
                    body_id_, castJPH(force), castJPH(point));
            }
            ...
            virtual void addImpulse(const float3& impulse,
                                    const float3& point) override {
                physics::Engine::bodyInterface()->AddImpulse(
                    body_id_, castJPH(impulse), castJPH(point));
            }
            ...
            virtual void setRestitution(f32 coeff) override {
                physics::Engine::bodyInterface()->SetRestitution(body_id_,
                                                                 coeff);
            }

            //! 跳ね返り係数を取得
            virtual f32 restitution() const override {
                return physics::Engine::bodyInterface()->GetRestitution(
                    body_id_);
            }
            ...
            virtual void setMotionType(physics::MotionType motion_type,
                ...
                physics::Engine::bodyInterface()->SetMotionType(
                    body_id_, convertTo(motion_type),
                    is_activate ? JPH::EActivation::Activate
                                : JPH::EActivation::DontActivate);
            }
            ...
            virtual matrix inverseInertia() const override {
                return castJPH(
                    physics::Engine::bodyInterface()->GetInverseInertia(
                        body_id_));
            }
    ...
    std::shared_ptr<physics::RigidBody> createRigidBody(
        ...
        // 作成
        return std::make_shared<RigidBodyImpl>(result.Get(), layer,
                                               motion_type);
    }
    ...
    //---------------------------------------------------------------------------
    std::shared_ptr<physics::RigidBody> createRigidBody(
        const shape::ConvexHull& o, u16 layer,
        physics::MotionType motion_type) {
        //----------------------------------------------------------
        // 頂点配列を取得
        //----------------------------------------------------------
    ...
    std::shared_ptr<physics::RigidBody> createRigidBody(const shape::Mesh& o,
        ...
        materialList.push_back(new JPH::PhysicsMaterialSimple(
            "BPMeshMaterial", JPH::Color::sGetDistinctColor(0)));

        //----------------------------------------------------------
        // シェイプを生成
        //----------------------------------------------------------
        JPH::MeshShapeSettings shape_settings(vertexList, indexList,
                                              materialList);
        JPH::ShapeSettings::ShapeResult result = shape_settings.Create();

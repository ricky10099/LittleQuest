...
namespace {
    ...
    constexpr u32 MAX_PAD_TYPE =
        static_cast<u32>(PAD_NO::PAD_NO_MAX);    //<! PAD接続最大数
    ...
    constexpr u32 PAD_TYPES[MAX_PAD_TYPE] = {DX_INPUT_PAD1, DX_INPUT_PAD2,
    ...
    for (int j = 0; j < MAX_PAD_TYPE; ++j) {
        ...
        for (int i = 0; i < MAX_PAD_NUM; ++i) {
            if (tmp_pad & PAD_BUTTONS[i]) {
                ++pads_[j][i];
                if (pads_[j][i] > UINT_MAX) pads_[j][i] = 1;
                continue;
            }

...
// シーン ローカル変数
//=============================================================
BP_BASE_IMPL_ABSOLUTE(Scene::Base, "シーン基底");

#pragma region Customize extern
...
#pragma region MyRegion
    if((/*IsPadOn(PAD_ID::PAD_10) ||*/ IsKeyDown(KEY_INPUT_ESCAPE)) && scene_can_pause) {
        Pause();
        select_num = 0;
    }
    ...
    if(scene_draw_menu) {
        HideMouse(false);
        if(IsKeyDown(KEY_INPUT_UP) || IsPadOn(PAD_ID::PAD_UP)) {
            --select_num;
        }
        if(IsKeyDown(KEY_INPUT_DOWN) || IsPadOn(PAD_ID::PAD_DOWN)) {
            ++select_num;
        }
        ...
        if(IsKeyDown(KEY_INPUT_RETURN) /* || IsPadOn(PAD_ID::PAD_3)*/) {
            menu_function[select_num]();
        }

        //if(IsPadOn(PAD_ID::PAD_2)) {
        //    Pause();
        //}
    }
...
void Scene::GUI() {
    bool is_select_ok =
        !ImGui::IsWindowHovered(ImGuiHoveredFlags_AnyWindow | ImGuiHoveredFlags_AllowWhenBlockedByActiveItem)    // Hover
    ...
    {
        ImGui::InputText("名前", debug_scene_name.data(), 1024, ImGuiInputTextFlags_EnterReturnsTrue);
        ...
        if(ImGui::TreeNode("登録シーン")) {
            ImGui::Text("シーン数 : %d", GetSceneCount());
            ImGui::Text("TODO: 存在シーン列挙");
            ImGui::Text("全Object数 : %d", Object::ExistObjectCount());

            if(leak_objs.size() > 0)
                ImGui::TextColored({1, 0, 0, 1}, "LeakObject数 : %d", leak_objs.size());
            ImGui::TreePop();
        }

        ImGui::DragFloat("経過時間", &scene_time, 0.1f, 0, 0, "%.2f");
        ImGui::Text("シーン内Object数 : %d", current_scene_->GetObjectPtrVec().size());
    ...
    for(int i = 0; i < size; i++) {
        ...
        if(obj->GetStatus(Object::StatusBit::ShowGUI)) {
            ...
            if(Scene::GetEditorStatus(Scene::EditorStatusBit::EditorPlacement)) {
                ...
                Scene::SetGUIWindow(Scene::SETGUI_ATTACH_RIGHT, (int)inspector_size.y, (int)object_detail_size.x,
...
void Scene::SetBGMVolume() {
    if(IsKeyDown(KEY_INPUT_LEFT) || IsPadOn(PAD_ID::PAD_LEFT)) {
        audio[0] -= 5;
    }
    if(IsKeyDown(KEY_INPUT_RIGHT) || IsPadOn(PAD_ID::PAD_RIGHT)) {
        audio[0] += 5;
    }

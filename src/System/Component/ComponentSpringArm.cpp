...
void ComponentSpringArm::Init() {
    ...
    Scene::GetCurrentScene()->SetPriority(shared_from_this(),
                                          ProcTiming::Update, Priority::LOWEST);
    // 向きはPostUpdateの最初で行っておく(カメラのPost向きよりも前になるように)
    Scene::GetCurrentScene()->SetPriority(
        shared_from_this(), ProcTiming::PostUpdate, Priority::HIGHEST);
...
matrix ComponentSpringArm::GetPutOnMatrix() const {
    float trans[3] = {0, 0, 0};
    float rot[3]   = {spring_arm_rotate_.x, spring_arm_rotate_.y,
...
void ComponentSpringArm::GUI() {
    ...
    ImGui::Begin(obj_name.data());
    {
        ...
        if (ImGui::TreeNode(u8"SpringArm")) {
            ...
            ImGui::DragFloat3(u8"SpringArm回転", (float*)&spring_arm_rotate_,
                              0.1f, -10000.0f, 10000.0f, "%.1f");
            ImGui::DragFloat3(u8"SpringArmオフセット",
                              (float*)&spring_arm_offset_, 0.1f, -10000.0f,
                              10000.0f, "%.1f");
            ImGui::DragFloat(u8"SpringArm長さ", (float*)&spring_arm_length_,
                             0.1f, -0.1f, 1000.0f, "%.1f");

            ImGui::DragFloat(u8"SpringArm 固さ", (float*)&spring_arm_strong_,
                             0.1f, 0.0f, 1.0f);
            ImGui::DragFloat(u8"SpringArm 戻り", (float*)&spring_arm_return_,
                             0.1f, 0.0f, 1.0f);

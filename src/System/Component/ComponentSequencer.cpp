...
matrix SequenceObject::GetTransformFromFrame(int frame) {
    ...
    auto calc_value = [](int frame, std::vector<uint32_t> keys,
    ...
    matrix mat;
    RecomposeMatrixFromComponents((float*)&position, (float*)&rotation,
...
void SequenceObject::ShowGuizmo() {
    ...
    if (show_guizmo_) {
        if (ShowGizmo(object_matrix_.f32_128_0, guizmo_operation_, guizmo_mode_,
                      reinterpret_cast<uint64_t>(this))) {
            float trns[3];
            float rot[3];
            float scale[3];
            DecomposeMatrixToComponents(object_matrix_.f32_128_0, trns, rot,
                                        scale);
        ...
        if (auto obj = Scene::GetObjectPtr<Object>(name_)) {
            ...
            if (auto component = obj->GetComponent<ComponentModel>()) {
                ...
                if (fabs(frame - animation_frame_old_) > 1.0f
                    && !component->IsPlaying())
                    component->PlayAnimation(animation_name_, false, 0.0f,
...
void SequenceObject::GUI(uint32_t start, uint32_t end) {
    if (ImGui::BeginNeoGroup(GetName().data(), &open_)) {
        ...
        // GUI ポジションキーの表示
        std::string pos_name =
            "Position##pos" + std::to_string(reinterpret_cast<uint64_t>(this));
        if (ImGui::BeginNeoTimeline<float3>(
                pos_name.data(), position_keys_, position_values_, nullptr,
            ...
            if (index >= 0 && position_keys_.size() > index) {
                ...
                ImGui::DragScalar(frame_table.data(), ImGuiDataType_U32,
                                  &position_keys_[index], 1, &start, &end);

                if (ImGui::DragFloat3("Position",
                                      (float*)&position_values_[index], 0.1f)) {
        ...
        // GUI ローテーションキーの表示
        std::string rot_name =
            "Rotation##rot" + std::to_string(reinterpret_cast<uint64_t>(this));
        if (ImGui::BeginNeoTimeline<float3>(
                rot_name.data(), rotation_keys_, rotation_values_, nullptr,
                ImGuiNeoSequencerFlags_AllowLengthChanging)) {
            int index = ImGui::GetNeoKeySelected();
            if (index >= 0 && rotation_keys_.size() > index) {
                ...
                std::string frame_table =
                    ...
                    + std::to_string(reinterpret_cast<uint64_t>(this));
                ImGui::DragScalar(frame_table.data(), ImGuiDataType_U32,
                                  &rotation_keys_[index], 1, &start, &end);

                if (ImGui::DragFloat3("Rotation",
                                      (float*)&rotation_values_[index], 0.1f)) {
        ...
        std::string scale_name =
            "Scale##scale" + std::to_string(reinterpret_cast<uint64_t>(this));
        if (ImGui::BeginNeoTimeline<float3>(
                scale_name.data(), scale_keys_, scale_values_, nullptr,
                ImGuiNeoSequencerFlags_AllowLengthChanging)) {
            int index = ImGui::GetNeoKeySelected();
            if (index >= 0 && scale_keys_.size() > index) {
                ...
                std::string frame_table =
                    ...
                    + std::to_string(reinterpret_cast<uint64_t>(this));
                ImGui::DragScalar(frame_table.data(), ImGuiDataType_U32,
                                  &scale_keys_[index], 1, &start, &end);

                if (ImGui::DragFloat3("Scale", (float*)&scale_values_[index],
                                      0.1f)) {
        ...
        // GUI アニメーションキーの表示
        std::string anm_name =
            "Animation##anm" + std::to_string(reinterpret_cast<uint64_t>(this));
        if (ImGui::BeginNeoTimeline<AnimObject>(
                anm_name.data(), animation_keys_, animation_values_, nullptr,
                ImGuiNeoSequencerFlags_AllowLengthChanging)) {
            int index = ImGui::GetNeoKeySelected();
            if (index >= 0 && animation_keys_.size() > index) {
                ...
                std::string frame_name =
                    "Frame##frame-anm"
                    + std::to_string(reinterpret_cast<uint64_t>(this));
                ImGui::DragScalar(frame_name.data(), ImGuiDataType_U32,
                                  &animation_keys_[index], 1, &start, &end);
        ...
        // GUI エフェクトキーの表示
        std::string eff_name =
            "Effect##effect" + std::to_string(reinterpret_cast<uint64_t>(this));
        if (ImGui::BeginNeoTimeline<AnimObject>(
                eff_name.data(), effect_keys_, effect_values_, nullptr,
                ImGuiNeoSequencerFlags_AllowLengthChanging)) {
            int index = ImGui::GetNeoKeySelected();
            if (index >= 0 && effect_keys_.size() > index) {
                ...
                std::string frame_name =
                    ...
                    + std::to_string(reinterpret_cast<uint64_t>(this));
                ImGui::DragScalar(frame_name.data(), ImGuiDataType_U32,
                                  &effect_keys_[index], 1, &start, &end);
                std::string check_name =
                    "Loop##checkbox-eff"
                    + std::to_string(reinterpret_cast<uint64_t>(this));
                ImGui::Checkbox(check_name.data(),
...
void ComponentSequencer::Update() {
    ...
    ImGui::Begin("Sequencer##__SequencerID__");
    {
        ...
        if (ImGui::BeginNeoSequencer(
                "Sequencer", &frame, &start_frame_, &end_frame_, {0, 0},
                ImGuiNeoSequencerFlags_AllowLengthChanging)) {

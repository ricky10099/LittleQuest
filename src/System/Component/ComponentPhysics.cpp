...
BP_COMPONENT_IMPL(ComponentPhysics, "Physics物理機能クラス");
...
void ComponentPhysics::GUI() {
    ...
    if(ImGui::TreeNode("Physics")) {
        ...
        // 形状
        {
            ...
            if(ImGui::Combo("型(Shape)", &shape, items, 6)) {
                // タイプが異なる場合は値もリセットする
                if(shape != shape_old) {
                    switch(shape) {
                    case 0:
            ...
            if(ImGui::Combo("タイプ(type)", &col_type, col_types, 16)) {
...

            {
...
#endif
                {
                    ...
                    value_change |= ImGui::DragFloat("質量(density)", (float*)&density_);
                    ...
                    case 1:
                        // to BOX
                        value_change |= ImGui::DragFloat3("大きさ(XYZ size)", (float*)&shape_data.box_size, 0.5f);
                    ...
                    case 2:
                        ...
                        value_change |=
                            (/* ImGui::DragFloat3( "sphere center", (float*)&shape_data.sphere_center, 0.1f ) || */
                             ImGui::DragFloat("半径(radius)", &shape_data.sphere_radius, 0.1f, 0.1f, 100.0f, "%.01f"));
                        if(value_change)
                            CreateSphere(shape_data.sphere_center, shape_data.sphere_radius, density_);
                        break;
                    ...
                    case 3:
                        // to Capsule
                        value_change |=
                            (ImGui::DragFloat("高さ(half height)", &shape_data.cylinder_half_height, 0.1f, 0.1f, 100.0f,
                                              "%.01f") ||
                             ImGui::DragFloat("半径(radius)", &shape_data.cylinder_radius, 0.1f, 0.1f, 100.0f, "%.01f"));
                        if(value_change)
                            CreateCapsule(shape_data.cylinder_half_height, shape_data.cylinder_radius, density_);
                        break;
            ...
            {
                // モデル姿勢
                if(ImGui::TreeNode(UNIQUE_TEXT("物理姿勢(matrix)"))) {
                    ImGui::DragFloat4(UNIQUE_TEXT("Ｘ軸"), physics_transform_.f32_128_0, 0.01f, -10000.0f, 10000.0f, "%.2f");
                    ImGui::DragFloat4(UNIQUE_TEXT("Ｙ軸"), physics_transform_.f32_128_1, 0.01f, -10000.0f, 10000.0f, "%.2f");
                    ImGui::DragFloat4(UNIQUE_TEXT("Ｚ軸"), physics_transform_.f32_128_2, 0.01f, -10000.0f, 10000.0f, "%.2f");
                    ImGui::DragFloat4(UNIQUE_TEXT("座標"), physics_transform_.f32_128_3, 0.01f, -10000.0f, 10000.0f, "%.2f");
                    ImGui::Separator();
                    ImGui::TreePop();
                }
            ...
            if(body_) {
                ...
                if(ImGui::DragFloat3(UNIQUE_TEXT("移動方向(LinearVelocity)"), (float*)&linear, 0.1f))
                    body_->setLinearVelocity(linear);

                if(ImGui::Button("動作リセット")) {
                ...
                if(ImGui::DragFloat("重力係数(gravityFactor)", &gravity_factor_, 0.05f, 0.0f, 5.0f))
                    body_->setGravityFactor(gravity_factor_);

                bool active = GetPhysicsStatus(PhysicsBit::Actived);
                ImGui::Checkbox("重力有効(Active)", &active);
                if(active) {
                    body_->setGravityFactor(gravity_factor_);
                    physics::Engine::bodyInterface()->ActivateBody(GetBodyID(body_));
                } else {
                ...
                if(ImGui::DragFloat("摩擦力(Friction)", &friction, 0.05f, 0.0f, 1000.0f))
                    body_->setFriction(friction);

                float restitution = body_->restitution();
                if(ImGui::DragFloat("跳返り係数(Restitution)", &restitution, 0.05f, 0.0f, 1.0f))
                    body_->setRestitution(restitution);

                u32* bit = &physics_status_.get();

                //ImGui::CheckboxFlags( u8"初期化済", &val, 1 << (int)PhysicsBit::Initialized );
                bool refresh = ImGui::CheckboxFlags("X軸回転禁止", bit, 1 << (int)PhysicsBit::LockRotX);
                refresh |= ImGui::CheckboxFlags("Y軸回転禁止", bit, 1 << (int)PhysicsBit::LockRotY);
                refresh |= ImGui::CheckboxFlags("Z軸回転禁止", bit, 1 << (int)PhysicsBit::LockRotZ);

...
class IMatrix {
	...
	const float* GetMatrixFloat() const
	{
        ...
        auto SetRotationToVectorWithLimit(float3 vec, float limit,
            ...
            if (rot_x > 0 && rot_x > limit) rot_x = limit;
            if (rot_x < 0 && rot_x < -limit) rot_x = -limit;

            if (rot_y > 0 && rot_y > limit) rot_y = limit;
            if (rot_y < 0 && rot_y < -limit) rot_y = -limit;

            if (rot_z > 0 && rot_z > limit) rot_z = limit;
            if (rot_z < 0 && rot_z < -limit) rot_z = -limit;
...
class ComponentTransform : public Component,
    ...
    public:
        ...
        virtual const matrix GetWorldMatrix() const override {
            if (world_transform_enable_) return world_transform_;

...
void Object::GUI() {
    ...
    ImGui::Begin(name_.c_str());
    {
        ...
        // 使用しているコンポーネント数
        ImGui::Text("使用コンポーネント数: %d", components_.size());

        char name[256];
        sprintf_s(name, "%s", name_.c_str());
        if(ImGui::InputText("名前", name, 256, ImGuiInputTextFlags_EnterReturnsTrue)) {
            name_default_ = name;
        ...
        if(ImGui::TreeNode("状態(Status)")) {
            ...
            ImGui::Checkbox("更新OFF", &noUpdate);
            ImGui::Checkbox("描画OFF", &noDraw);
            ImGui::Checkbox("PAUSE禁止", &disPause);
...
BP_OBJECT_BASE_IMPL(Object, "Objectクラス");

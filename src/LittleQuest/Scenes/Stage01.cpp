...
BP_CLASS_IMPL(Stage01, "LittleQuest/Stage01")
...
void Stage01::Update() {
    ...
    case Scene::SceneState::TRANS_IN:
        ...
        if(IsKeyDown(KEY_INPUT_RETURN) || IsMouseDown(MOUSE_INPUT_1) || IsKeyDown(KEY_INPUT_SPACE)    // ||
           /*IsPadDown(PAD_ID::PAD_10, DX_PADTYPE_DUAL_SENSE) || */ /* IsPadOn(PAD_ID::PAD_3)*/) {
            m_fadeTimer     = 0;
            m_alpha         = 0;
            m_cutSceneTimer = 0;
    ...
    case Scene::SceneState::TRANS_OUT:
        if(FadeIn()) {
            if(ShowMessage()) {
                ...
                if(IsKeyDown(KEY_INPUT_RETURN) || IsMouseDown(MOUSE_INPUT_1) || IsKeyDown(KEY_INPUT_SPACE) ||
                   IsKeyDown(KEY_INPUT_ESCAPE) /* || IsPadOn(PAD_ID::PAD_10) ||
                   IsPadOn(PAD_ID::PAD_3)*/
                   || m_endingTimer <= 0) {
